...
func TestTerraformAwsRdsExample(t *testing.T) {
	...
	ttable := []struct {
	...
	}{
		{
			...
			schemaCheck: func(t *testing.T, dbURL string, dbPort int32, dbUsername, dbPassword, expectedSchemaName string) bool {
				...
				return aws.GetWhetherSchemaExistsInRdsMySQLInstanceContext(t, context.Background(), dbURL, dbPort, dbUsername, dbPassword, expectedSchemaName)
			},
			expectedOptins: map[struct {
				opName  string
				setName string
		...
		},
		{
			...
			schemaCheck: func(t *testing.T, dbURL string, dbPort int32, dbUsername, dbPassword, expectedSchemaName string) bool {
				...
				return aws.GetWhetherSchemaExistsInRdsPostgresInstanceContext(t, context.Background(), dbURL, dbPort, dbUsername, dbPassword, expectedSchemaName)
			},
		},
	}
	...
	for _, tt := range ttable {
		t.Run(tt.name, func(t *testing.T) {
			...
			engineVersion := aws.GetValidEngineVersionContext(t, context.Background(), awsRegion, tt.engineName, tt.majorEngineVersion)
			instanceType := aws.GetRecommendedRdsInstanceTypeContext(t, context.Background(), awsRegion, tt.engineName, engineVersion, []string{"db.t2.micro", "db.t3.micro", "db.t3.small"})
			...
			address := aws.GetAddressOfRdsInstanceContext(t, context.Background(), dbInstanceID, awsRegion)
			port := aws.GetPortOfRdsInstanceContext(t, context.Background(), dbInstanceID, awsRegion)
			...
			for k, v := range tt.expectedParameter {
				assert.Equal(t, v, aws.GetParameterValueForParameterOfRdsInstanceContext(t, context.Background(), k, dbInstanceID, awsRegion))
			}

			// assert all parameters
			params := aws.GetAllParametersOfRdsInstanceContext(t, context.Background(), dbInstanceID, awsRegion)
			...
			for k, v := range tt.expectedOptins {
				...
				assert.Equal(t, v, aws.GetOptionSettingForOfRdsInstanceContext(t, context.Background(), k.opName, k.setName, dbInstanceID, awsRegion))
			}
		})
	}
}
