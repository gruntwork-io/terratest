...
func FetchContentsOfFileFromInstance(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, instanceID string, useSudo bool, filePath string) string {
...
// fetches the contents of the file at the given path (using sudo if useSudo is true), and returns the contents of
// that file as a string.
func FetchContentsOfFileFromInstanceE(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, instanceID string, useSudo bool, filePath string) (string, error) {
...
func FetchContentsOfFilesFromInstance(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, instanceID string, useSudo bool, filePaths ...string) map[string]string {
...
// fetches the contents of the files at the given paths (using sudo if useSudo is true), and returns a map from file path
// to the contents of that file as a string.
func FetchContentsOfFilesFromInstanceE(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, instanceID string, useSudo bool, filePaths ...string) (map[string]string, error) {
	publicIp, err := GetPublicIpOfEc2InstanceE(t, instanceID, awsRegion)
	if err != nil {
		return nil, err
	}
...
func FetchContentsOfFileFromAsg(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, asgName string, useSudo bool, filePath string) map[string]string {
...
// Override SSH Agent auth methods), fetches the contents of the file at the given path (using sudo if useSudo is true),
// and returns a map from Instance ID to the contents of that file as a string.
func FetchContentsOfFileFromAsgE(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, asgName string, useSudo bool, filePath string) (map[string]string, error) {
	...
	for _, instanceID := range instanceIDs {
		contents, err := FetchContentsOfFileFromInstanceE(t, awsRegion, sshOpts, instanceID, useSudo, filePath)
		if err != nil {
			return nil, err
		}
...
func FetchContentsOfFilesFromAsg(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, asgName string, useSudo bool, filePaths ...string) map[string]map[string]string {
...
// Override SSH Agent auth methods), fetches the contents of the files at the given paths (using sudo if useSudo is true),
// and returns a map from Instance ID to a map of file path to the contents of that file as a string.
func FetchContentsOfFilesFromAsgE(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, asgName string, useSudo bool, filePaths ...string) (map[string]map[string]string, error) {
	instanceIDs, err := GetInstanceIdsForAsgE(t, asgName, awsRegion)
	if err != nil {
		return nil, err
	}
	...
	for _, instanceID := range instanceIDs {
		contents, err := FetchContentsOfFilesFromInstanceE(t, awsRegion, sshOpts, instanceID, useSudo, filePaths...)
		if err != nil {
			return nil, err
		}
...
func FetchFilesFromInstance(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, instanceID string, useSudo bool, remoteDirectory string, localDirectory string, filenameFilters []string) {
...
// Override SSH Agent auth methods), downloads the files matching filenameFilters at the given remoteDirectory
// (using sudo if useSudo is true), and stores the files locally at localDirectory/<publicip>/<remoteFolderName>
func FetchFilesFromInstanceE(t testing.TestingT, awsRegion string, sshOpts *ssh.AuthOptions, instanceID string, useSudo bool, remoteDirectory string, localDirectory string, filenameFilters []string) error {
	publicIp, err := GetPublicIpOfEc2InstanceE(t, instanceID, awsRegion)
...
// username and one or more of: Key Pair, SSH Agent or Override SSH Agent auth methods, downloads the files
