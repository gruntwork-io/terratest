...
func TestFetchRecoveryServicesVault_Success(t *testing.T) {
	...
	srv := recoveryfake.VaultsServer{
		Get: func(ctx context.Context, resourceGroupName, vaultName string, options *armrecoveryservices.VaultsClientGetOptions) (resp azfake.Responder[armrecoveryservices.VaultsClientGetResponse], errResp azfake.ErrorResponder) {
	...
	vault, err := fetchRecoveryServicesVault(context.Background(), client, "rg", "testvault")
	require.NoError(t, err)
...
func TestFetchRecoveryServicesVault_NotFound(t *testing.T) {
	...
	client := newFakeVaultsClient(t, srv)
	_, err := fetchRecoveryServicesVault(context.Background(), client, "rg", "missing")
	require.Error(t, err)
...
func TestCollectBackupPolicies_OnePage(t *testing.T) {
	...
	srv := backupfake.BackupPoliciesServer{
		NewListPager: func(vaultName, resourceGroupName string, options *armrecoveryservicesbackup.BackupPoliciesClientListOptions) (resp azfake.PagerResponder[armrecoveryservicesbackup.BackupPoliciesClientListResponse]) {
	...
	policies, err := collectBackupPolicies(context.Background(), client, "vault", "rg")
	require.NoError(t, err)
...
func TestCollectBackupProtectedVMs_WithVMItems(t *testing.T) {
	...
	srv := backupfake.BackupProtectedItemsServer{
		NewListPager: func(vaultName, resourceGroupName string, options *armrecoveryservicesbackup.BackupProtectedItemsClientListOptions) (resp azfake.PagerResponder[armrecoveryservicesbackup.BackupProtectedItemsClientListResponse]) {
	...
	vms, err := collectBackupProtectedVMs(context.Background(), client, "vault", "rg", "policy1")
	require.NoError(t, err)
	assert.Len(t, vms, 2)
