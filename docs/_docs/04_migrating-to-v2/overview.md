...
Note that v2 modules are versioned and released in lockstep, so all 16
carry the same version. Pin them together.

## The four changes

**1. Import paths gain a `/v2` suffix.** The semantic import version goes
after the module root, not the end of the path:
...
**2. Six utility packages collapse into `core`.** `random`, `files`,
`formatting`, `logger`, `shell`, `retry` and `testing` are no longer
separate packages:
...
**3. Three packages are renamed to drop the hyphen.** This changes the
...
**4. Each module needs its own `require`.** Where v1 was one line in
`go.mod`, v2 needs one per module you import.
...
1. Rewrite import paths and package identifiers. This is mechanical and the
...
Steps 1 to 3 are safe: if it builds, you are done. Step 4 is the one that
needs reading.
...
Function names, arguments and return types are unchanged except for the
relocations noted above. The `Foo` / `FooE` convention is unchanged. Test
data written by v1 loads in v2: filenames and JSON layout are the same.

If you already migrated to v1 and adopted the `Context` variants, there is
nothing further to do on that front. If you have not, see [migrating to
v1]({{site.baseurl}}/docs/migrating-to-v1/overview/) first; doing both at
once is harder than doing them in sequence.
